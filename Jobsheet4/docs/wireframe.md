...
belum mencakup fitur Login, Dashboard Petugas, dan Peminjaman/Pengembalian.

## 1. Pengguna

1. **User (Pengguna Umum)**:
   - perlu melakukan login.
   - Memiliki Izin baca dan dapat melihat katalog buku publik.
   - Tidak memiliki akses ke data anggota dan tidak dapat memanipulasi data (tambah/edit/hapus buku), maupun transaksi sirkulasi peminjaman.

2. **Petugas Perpustakaan (Administrator)**:
   - Wajib login menggunakan kredensial.
   - Memiliki hak CRUD: Create, Read, Update, Delete terhadap katalog buku dan data anggota.
   - Memiliki hak eksklusif untuk menjalankan transaksi: mencatat peminjaman baru, memproses pengembalian buku, dan memeriksa riwayat peminjaman per anggota.
